...
function LoanRejectTable({ handle }) {
  ...
  const filterRejectLoans = async () => {
    ...
    try {
      const response = await filterRejectedLoansByDate(
        fromDate,
        toDate,
        leadFirstName
      );
      ...
      console.error("Error filtering Rejected loans:", error);
      alert("An error occurred while filtering Rejected loans.");
    }
  };
  const resetFilters = () => {
    setSearchForm({
  ...
  if (leadsData == null) {
    return (
      <h2 className="m-4 text-white text-[21px] font-semibold font-mono bg-red-500 rounded-md p-2 m-2">
      ...
      {!isLeadDetailFrame && (
        <div className="relative h-[85%] overflow-hidden">
          <div className="px-4 bg-gradient-to-r from-[#e43364] to-[#3858f9] flex flex-row justify-items-start items-center">
            <h2 className=" text-[16px]  font-sans font-bold  text-white ">
              All Rejected Loans
            </h2>
            ...
            <div>
              ...
              <input
                ...
                className="text-black h-10 text-[18px] font-mono p-2 m-1 rounded-md outline-none border-[3px] border-indigo-200"
          ...
          <div className="h-[503px] overflow-auto">
