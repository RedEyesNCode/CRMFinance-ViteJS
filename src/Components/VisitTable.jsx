    ...
    <main className="relative h-full w-full overflow-y-scroll">
      <div className="rounded-2xl border border-gray-300 border-r-0 h-full w-full">
        {VisitData ? (
          ...
          <div className="text-white text-[21px] font-semibold font-mono bg-blue-800 px-5 py-3 rounded-md flex justify-between items-center">
            No Visit Found !!
            <button
              onClick={() => setaddVisit(true)}
              className="backdrop-blur-md text-lg font-normal border px-2 rounded-xl py-1"
        ...
        <div className="fixed z-10 inset-0 overflow-y-auto flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-lg p-8 max-w-sm">
            ...
            <div className="flex justify-end">
              <button
                onClick={() => handleHideDeleteVisit(CurrentVisit)}
                className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md mr-2"
              >
                Cancel
              </button>
              <button
                onClick={() => deleteCurrentVisit(CurrentVisit)}
      ...
      {addVisit && (
        <>
          <CreateVisit close={() => setaddVisit(false)} />
          <p
            className="absolute top-10 left-[70%] cursor-pointer"
